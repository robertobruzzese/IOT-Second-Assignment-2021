...
printf("punto 1 ");
 ...
 //   printf("gw.addr.ipv6  %g", &gw.addr.ipv6);
...
//        int c = strftime(datetime, sizeof(datetime), "%Y-%m-%d %T", tt);
 ...
 //       } 
  printf("punto 1b ");   
       ...
       sprintf(json, "{\"id\": \"%d\",  \"accX\": %6i, \"accY\": %6i,\"accZ\": %6i}",
                 atoi(value), acc_value.x_axis, acc_value.y_axis, acc_value.z_axis);  
 ...
 printf("punto 1ca");
  ...
  //      xtimer_periodic_wakeup(&last, DELAY);       
           ...
           sprintf(json, "{\"id\": \"%d\",  \"magX\": %6i, \"magY\": %6i,\"magZ\": %6i}",
                 atoi(value), mag_value.x_axis, mag_value.y_axis, mag_value.z_axis);  
...
// it sleeps for five seconds
 ...
 //       } 
        ...
        sprintf(json, "{\"id\": \"%d\",  \"X\": %6i, \"Y\": %6i,\"Z\": %6i}",
                  atoi(value),  acc_data.acc_x, acc_data.acc_y, acc_data.acc_z);                  
...
static void _sensors_usage(char *cmd)
{
    printf("usage: %s <start|stop>\n", cmd);
}

static int sensors_handler(int argc, char *argv[])
{
    if (argc < 2) {
        _sensors_usage(argv[0]);
        return -1;
    }

    /* Implement the sensors start/stop subcommands here */
...
static int netif_add(char *iface_name,char *addr_str)
{
  ...
  //  netif_t *iface = netif_get_by_id(iface_name);
         if (!iface) {
...
static const shell_command_t commands[] = {
    ...
    { "l3g", "read the gyroscope l3g4200d values", l3g4200d_handler },
     ...
     /* l3g4200d shell command handler */
    { "sensors", "start sensors reading loop", sensors_handler },
...
/*adding an ipv6 address interface */     
